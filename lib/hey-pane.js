...
export default {
  ...
  FollowObserver: null,
  modifiedPanes: [],
  incompatiblePackages: ['nuclide'],

  activate(state) {
    this.subscriptions = new CompositeDisposable()

    this.subscriptions.add(atom.packages.onDidActivatePackage(
      this.checkIncompatiblePackages.bind(this)))
  ...
  checkIncompatiblePackages(package) {
    if (this.incompatiblePackages.includes(package.name)) {
      atom.notifications.addError('Incompatible Package Detected',
        {
          dismissable: true,
          detail: `hey-pane does not work when package "${package.name}" is activated.`
        }
      )
    }
  }
