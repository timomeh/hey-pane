...
export default {
  ...
  config: {
    ...
    followLocations: {
      ...
      description: 'Specifies the locations within which follow mode is enabled',
      type: 'object',
      collapsed: true,
      properties: {
        center: {
  ...
  toggleFollow() {
    ...
    if (targets.center) targetLocations.push('center')
    if (targets.left) targetLocations.push('left')
    if (targets.bottom) targetLocations.push('bottom')
    if (targets.right) targetLocations.push('right')
    this.FollowObserver = atom.workspace.onDidStopChangingActivePaneItem(item => {
      const pane = atom.workspace.paneContainerForItem(item)
      if (!pane) return
      if (targetLocations.includes(pane.getLocation())) {
        this.startFollow()
      }
    })
  },
